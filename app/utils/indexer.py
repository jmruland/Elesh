    ...
    while time.time() - start < timeout:
        ...
        try:
            ...
            if r.status_code == 200:
                logger.info(f"Ollama is ready and embedding model is responsive!")
                return
            else:
...
def load_or_create_index():
    ensure_data_dirs()
    logger.info(f"Starting lore/rulebooks index loading...")
    wait_for_ollama()
    try:
        ...
        docs = get_documents()
        if not docs:
            logger.warning("No documents to index. Archivist will start with no data.")
            return None
